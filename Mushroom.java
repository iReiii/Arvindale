...
public class Mushroom extends Actor{
    ...
    private void loadImages() {
        for (int i = 0; i <= 3; i++) {
            idleRightImages.add(scaleImage(new GreenfootImage("Character/Mushroom/Mushroom_idle/Mushroom_Idle" + i + ".png")));
        }
        for (int i = 0; i <= 7; i++) {
            runRightImages.add(scaleImage(new GreenfootImage("Character/Mushroom/Mushroom_run/Mushroom_Run" + i + ".png")));
        }
        for (int i = 0; i <= 3; i++) {
            hurtRightImages.add(scaleImage(new GreenfootImage("Character/Mushroom/Mushroom_hurt/Mushroom_Hurt" + i + ".png")));
        }
        for (int i = 0; i <= 7; i++) {
            attackRightImages.add(scaleImage(new GreenfootImage("Character/Mushroom/Mushroom_attack/Mushroom_Attack" + i + ".png")));
        }
        for (int i = 0; i <= 3; i++) {
            deathRightImages.add(scaleImage(new GreenfootImage("Character/Mushroom/Mushroom_death/Mushroom_Death" + i + ".png")));
        }
        mirrorImages();
    }
    ...
    private void chaseNightBorne() {
        ...
        if (nightBorne != null) {
            ...
            if (mushroomX < nightBorneX) {
                facingDirection = "right";
                setLocation(getX() + speed, getY());
            } else if (mushroomX > nightBorneX) {
                facingDirection = "left";
                setLocation(getX() - speed, getY());
            }
            
            // Cek jarak untuk serangan
            if (Math.abs(mushroomX - nightBorneX) < attackRange) {
                isAttacking = true;
                frameIndex = 0;
            }
        }
